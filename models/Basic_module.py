...
class Balanced_DiceCELoss(DiceCELoss):
    ...
    def forward(self, input, target):
        ...
        weight = torch.zeros(num_classes, device=input.device)

        unique_classes, counts = torch.unique(target, return_counts=True)
        weight[unique_classes.long()] = counts.float()

        weight = 1.0 - weight / target.numel()
